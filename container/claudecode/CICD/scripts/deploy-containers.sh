...
BACKEND_JAR_PATH="${PROJECT_ROOT}/sample-app/backend/target"
...
pre_deployment_checks() {
    ...
    log_variable "docker-compose.yml" "${PROJECT_ROOT}/docker-compose.yml"
}
...
build_containers() {
    ...
    cd "$PROJECT_ROOT"
    log_variable "作業ディレクトリ" "$(pwd)"
    log_variable "ビルドコンテキスト(backend)" "${PROJECT_ROOT}/sample-app/backend"
    log_variable "ビルドコンテキスト(nginx)" "${PROJECT_ROOT}/sample-app"
