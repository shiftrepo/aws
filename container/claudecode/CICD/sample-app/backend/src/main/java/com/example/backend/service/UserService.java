...
public class UserService {
    ...
    public List<UserDto> findByDepartmentId(Long departmentId) {
        ...
        if (!departmentRepository.existsById(departmentId)) {
            throw new EntityNotFoundException("部門が見つかりません。ID: " + departmentId);
        }
    ...
    public UserDto create(UserDto dto) {
        ...
        if (userRepository.existsByEmail(dto.getEmail())) {
            throw new IllegalArgumentException("メールアドレスが既に使用されています: " + dto.getEmail());
        }
    ...
    private UserDto convertToDto(User user) {
        ...
        if (user.getDepartment() != null) {
            ...
            // 組織情報を設定
            if (user.getDepartment().getOrganization() != null) {
                builder.organizationId(user.getDepartment().getOrganization().getId());
                builder.organizationName(user.getDepartment().getOrganization().getName());
            }
        }
