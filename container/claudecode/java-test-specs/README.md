...
- `--source-dir ./src/test/java` のみを指定すると、カバレッジレポートが見つかりません（テストファイルのみの処理になります）
- カバレッジデータを含める場合は必ずプロジェクトルートを指定してください
...
| `--csv-output` | - | CSV形式でのテスト仕様書も生成（Excel出力に追加） | false |
...
**重要な注意点**:
- `--source-dir .` を指定してプロジェクトルート全体を処理し、テスト + カバレッジ + 実行結果を取得
- カバレッジを含める場合は、事前に `coverage-reports` ディレクトリを作成してください（上記の例を参照）
- CSV出力が必要な場合は `--csv-output` オプションを追加してください
