...
from app.patent_system.models import (
    ...
    Claim, Description, SessionLocal, get_db
)
...
class PatentAnalyzer:
    """Patent data analysis utilities"""
    ...
    def analyze_technology_trends(self, years: int = 10, top_n: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            # SQL to get IPC trends over time
            sql = text("""
                SELECT 
                ...
                WHERE 
                    ...
                    EXTRACT(YEAR FROM p.application_date) >= :start_year
                GROUP BY year, ipc_code
                ORDER BY year DESC, patent_count DESC
            """)
            
            # Execute query
            result = self.db.execute(sql, {"start_year": start_year})
            ...
            logger.error(f"Error in technology trend analysis: {str(e)}")
            return {"error": str(e)}
    ...
    def analyze_applicant_competition(self, top_n: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error in applicant competition analysis: {str(e)}")
            return {"error": str(e)}
    ...
    def analyze_patent_landscape(self, ipc_level: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error in patent landscape analysis: {str(e)}")
            return {"error": str(e)}
    ...
    def generate_analysis_report(self) -> str:
        ...
        try:
            ...
            applicant_count = self.db.query(Applicant.name.distinct()).count()
            inventor_count = self.db.query(Inventor.name.distinct()).count()
            
            # Get date range of patents
            earliest_date_query = self.db.query(Patent.application_date).filter(
                Patent.application_date != None
            ).order_by(Patent.application_date.asc()).first()
            
            latest_date_query = self.db.query(Patent.application_date).filter(
                Patent.application_date != None
            ).order_by(Patent.application_date.desc()).first()
            
            earliest_date = earliest_date_query[0] if earliest_date_query else None
            latest_date = latest_date_query[0] if latest_date_query else None
